...
const DEFAULT_SEND_EPSILON = 0.0000001;
...
function tagToTrackType(tag: string): TrackType | null {
  if (tag === 'AudioTrack') return 'audio';
  if (tag === 'MidiTrack') return 'midi';
  if (tag === 'GroupTrack') return 'group';
  if (tag === 'ReturnTrack') return 'return';
  return null;
}
...
function isTrackDescendantOf(
  ...
  while (groupId) {
    if (groupId === ancestorGroupId) return true;
...
function resolveRestoreClosure(
  ...
  for (const id of included) {
    const track = map.get(id);
    if (!track || track.type === 'return') continue;
  ...
  for (const index of neededReturnIndices) {
    ...
    if (returnTrack) included.add(returnTrack.id);
  }
...
function remapIdsInClone(
  ...
  $(root)
    ...
    .each((_, node) => {
      const raw = $(node).attr('Id');
      if (!raw || node.tagName === 'TrackSendHolder') return;
      if (!idMap.has(raw)) idMap.set(raw, allocId());
...
function normalizeTrackSends(
  ...
  if (originalHolders.length === 0) return;
  ...
  for (let i = 0; i < targetReturnCount; i++) {
    if (!byTargetIndex.has(i)) {
      ...
      if (filler) byTargetIndex.set(i, filler);
    }
  }
...
export async function smartRestoreTracks(input: {
  ...
  for (const sourceReturn of sourceReturns) {
    ...
    if (!includedIds.has(sourceReturn.id)) continue;
  ...
  for (const track of normalTracksToInsert) {
    ...
    if (newTrackId) trackIdMap.set(track.id, newTrackId);
  ...
  if (firstReturnNode.length > 0) {
    for (const xml of rewrittenNormalXml) firstReturnNode.before(xml);
  } else {
    for (const xml of rewrittenNormalXml) tracksNode.append(xml);
  }
  for (const xml of rewrittenReturnXml) tracksNode.append(xml);
