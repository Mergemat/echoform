...
describe("EchoformService file-bound branches", () => {
  ...
  test("branching from a save creates a new branch file and opens it", async () => {
    ...
    const firstSave = expectSave(first);
  ...
  test("tracking and saving ignore Ableton Backup directories", async () => {
    ...
    expect(createdSave.metadata.fileCount).toBe(1);
    expect(createdSave.metadata.sizeBytes).toBe("version-1".length);
